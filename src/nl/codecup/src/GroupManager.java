...
public class GroupManager {
	...
	public Group[] mergeGroupsByMove(Group[] groups, Board board, Move move) {
		this.groups = groups;
		this.board = board;
		checkGroupsForMove(move.getOriginXConverted(), move.getOriginYConverted(), move.getTargetXConverted(),
    ...
    private Group mergeGroups(Group[] groups) {
        ...
        groups[mergedGroup.getIndexInList()] = mergedGroup;
        return mergedGroup;
    }
    ...
    private void setPlayerGroupNull(Group group) {
        if (group != null && Arrays.asList(groups).contains(group)) {
            groups[group.getIndexInList()] = null;
        }
    }
