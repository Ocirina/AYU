	...
	public Move chooseMove() {
		...
		for (int column = 0; column < contentLength; column++) {
			for (int row = 0; row < contentLength; row++) {
				 ...
				 * W | |
				 * 
				 * Situation 3 Start: | | | W | W | W | | W | | Result: | | | |
				 * W | W | W | W | |
				 */
				boolean gapScenario = (column <= 8
						&& content[row][column] == piece
						&& content[row + 1][column] == 0 && content[row + 2][column] == piece);
				...
				if (gapScenario) {
					...
					if (columnPieceToMove > 0) {
						return new Move(row, columnPieceToMove + 1, row,
								column + 1);
					}
					// TODO: validMoveCheck?
				}
